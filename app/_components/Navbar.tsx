...
import { Menu, X } from "lucide-react"; // lucide-react ব্যবহার করে আইকন
          ...
          <div className="hidden lg:flex items-center gap-10">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.href}
                className="relative text-[16px] font-bold text-slate-600 hover:text-[#0F172A] transition-colors group"
              >
                {link.name}
                <span
                  className={`absolute -bottom-1 left-0 w-0 h-0.5 bg-gradient-to-r from-[#6FDEF7] to-[#2F8BDD] transition-all duration-300 group-hover:w-full ${
                    link.name === "Home" ? "w-full" : ""
                  }`}
                />
              </Link>
            ))}
          ...
          >
            {navLinks.map((link) => (
              <Link
                ...
                className="text-xl font-bold text-slate-700 hover:text-[#2F8BDD] transition-colors"
              >
                {link.name}
              </Link>
            ))}
